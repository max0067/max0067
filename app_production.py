"""
Version de production de l'application optimisée pour o2switch
Utilisez ce fichier comme référence ou renommez-le en app.py pour la production
"""
...
if not app.debug:
    ...
    app.logger.info('Application RSS Manager démarrée')
...
# Note: Sur un hébergement mutualisé, préférez les tâches cron
if app.config.get('SCHEDULER_ENABLED', True):
    try:
        ...
        scheduler.add_job(
            ...
            trigger=IntervalTrigger(minutes=app.config.get('UPDATE_INTERVAL_MINUTES', 30)),
            id='update_feeds_job',
        ...
        app.logger.error(f'Erreur lors de l\'initialisation du scheduler: {str(e)}')
...
def api_get_feeds():
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la récupération des flux: {str(e)}")
...
def api_get_feed(feed_id):
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la récupération du flux: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_add_feed():
    ...
    try:
        ...
        if feed_id:
            ...
            app.logger.info(f"Nouveau flux ajouté (ID: {feed_id})")
            try:
                update_single_feed(feed_id)
            except Exception as e:
                app.logger.error(f"Erreur lors de la mise à jour initiale: {str(e)}")
        ...
        app.logger.error(f"Erreur lors de l'ajout du flux: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_update_feed(feed_id):
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la mise à jour du flux: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_delete_feed(feed_id):
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la suppression du flux: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_update_all_feeds():
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la mise à jour des flux: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_get_articles():
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la récupération des articles: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
def api_mark_article_read(article_id):
    ...
    try:
        ...
        app.logger.error(f"Erreur lors de la mise à jour de l'article: {str(e)}")
        return jsonify({'success': False, 'error': 'Erreur serveur'}), 500
...
# Ce bloc n'est pas utilisé avec Passenger, mais utile pour les tests locaux
